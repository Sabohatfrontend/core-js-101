...
function doRectanglesOverlap(rect1, rect2) {
  const r1X = rect1.left + rect1.width;
  const r1Y = rect1.top + rect1.height;
  const r2X = rect2.left + rect2.width;
  const r2Y = rect2.top + rect2.height;
  if (rect1.left === r1X || rect1.top === r1Y) { return false; }
  if (r2X === rect2.left || rect2.top === r2Y) { return false; }
  if (rect1.left > r2X || rect2.left > r1X) {
    return false;
  }
  if (r1Y < rect2.top || r2Y < rect1.top) {
    return false;
  }
  return true;
}
...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (chars.length === 1) return str[i];
  }
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const nums = [a, b].sort((aa, bb) => aa - bb);
  let str = '';
  str += isStartIncluded ? '[' : '(';
  str += nums.join(', ');
  str += isEndIncluded ? ']' : ')';
...
function isCreditCardNumber(ccn) {
  ...
  let sum = 0;
  const parity = ccnArr.length % 2;
  for (let i = 0; i < ccnArr.length; i += 1) {
    if (i % 2 !== parity) sum += Number(ccnArr[i]);
    else if (Number(ccnArr[i]) > 4) sum += 2 * Number(ccnArr[i]) - 9;
    else { sum += 2 * Number(ccnArr[i]); }
  }
  return sum % 10 === 0;
}
/**
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (OPENBR.includes(currentSymbol)) stack.push(currentSymbol);
    else {
      if (stack.length === 0) return false;
      const topElement = stack[stack.length - 1];
      if (BR[currentSymbol] === topElement) stack.pop();
      else return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  let index = 0;
  while (t) {
    index = pathes[0].indexOf('/', index);
    if (index === -1) return sum;
    cmPath = pathes[0].slice(0, index + 1);
    for (let i = 1; i < pathes.length; i += 1) {
      if (pathes[i].slice(0, index + 1) !== cmPath) return sum;
    }
    sum = cmPath;
    index += 1;
  }
